...
set -e
log_path="/Library/Addigy/PIT Pro/mbbr"
...
if [ "$username" != "" ]; then
/Library/Addigy/macmanage/MacManage.app/Contents/MacOS/MacManage action=notify title="Malware Found" description="Maintenance has found Malware. Restart your Mac immediately." acceptLabel="Restart" && sudo -u $username osascript -e 'tell app "loginwindow" to Â«event aevtrrstÂ»'
exit 1
else
reboot
exit 1
