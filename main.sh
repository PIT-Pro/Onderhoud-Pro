...
/Library/Addigy/macmanage/MacManage.app/Contents/MacOS/MacManage action=notify title="PIT Pro Maintenance" description="Click on the button below to start." closeLabel="Run Maintenance"  && proceed=1
...
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/PIT-Pro/Maintenance/main/Files/clear_browser_caches.sh)" >> "$LOGFILE"
echo "Kickstarting softwareupdated" >> "$LOGFILE"
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/PIT-Pro/Maintenance/main/Files/softwareupdated.sh)" >> "$LOGFILE"
